...
void DiskHeadManager::generateTasksForDisk(int diskId) {
    ...
    while (!diskReadUnits[diskId].empty() && availableTokens > 0) {
        ...
        //     }
            ...
            // 比较总损失，选择最优方案
            if (ifUseReadPlan(diskId, currentPos, nextUnit)) {
                ...
                headStates[diskId].lastAction = ACTION_READ;
                if (currentPos != (nextUnit % unitCount) + 1) 
                    break;
...
bool DiskHeadManager::ifUseReadPlan(int diskId, int currentPos, int nextPos) {
    if (headStates[diskId].lastAction != ACTION_READ) {
        return false;
    }
    ...
    int tmpNextPos = nextPos;

    int lastReadCost = headStates[diskId].lastTokenCost;

    // 向后看3个要读取的单元
    for (int i = 0; i < 4; i++) {
        int passCount = calculatePassCount(tmpCurrentPos, tmpNextPos);

        passPlanCost += (passCount + 64);

        for (int j = 0; j <= passCount; j++) {
            int cost = std::max(16, (int)std::ceil(lastReadCost * 0.8));
            lastReadCost = cost;
            readPlanCost += cost;
        }

        if (readPlanCost < passPlanCost){
            std::cerr << "readPlanCost: " << readPlanCost << " passPlanCost: " << passPlanCost << std::endl;
            return true;
        }
            

        tmpCurrentPos = (tmpCurrentPos + passCount) % unitCount +1;
        tmpNextPos = findNextReadUnit(diskId, tmpCurrentPos);
    }
    return false;
}